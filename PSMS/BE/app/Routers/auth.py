...
from ..Schemas import Token, UserCreate, UserOut, RefreshTokenRequest
...
def register(payload: UserCreate, db: Session = Depends(get_db)):
    ...
    target_role_name = (payload.role or "citizen").lower()
    role = db.query(models.Role).filter(models.Role.name == target_role_name).first()
    if not role:
        raise HTTPException(status_code=400, detail="Role is not available")

    if target_role_name != "citizen":
        expected_secret = settings.ROLE_REGISTRATION_SECRETS.get(target_role_name)
        if not expected_secret or payload.role_secret != expected_secret:
            raise HTTPException(status_code=403, detail="Invalid role secret")
...
def logout(payload: RefreshTokenRequest, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    revoked = revoke_refresh_token(db, payload.refresh_token, user_id=current_user.id)
    return {"status": "ok"}
